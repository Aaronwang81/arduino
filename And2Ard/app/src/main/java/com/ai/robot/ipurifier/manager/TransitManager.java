...
public class TransitManager {
    ...
    public Handler _uiEventHandler = null;


    public interface ITransitCallback {
        //收到数据的回调，保证在主线程(Loop.getMainLooper())中执行
    ...
    public TransitManager(Context context){
        ...
        _thread.start();
        _uiEventHandler = new Handler(Looper.getMainLooper());
    ...
    private void dataReceived(final Integer deviceID, final byte[] buffer){
            _uiEventHandler.post(new Runnable() {
                @Override
                public void run() {
